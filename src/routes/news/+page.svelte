...
<div class="container mx-auto p-4">
  <h1 class="text-2xl font-bold mb-4">Hacker News Top 25 Stories</h1>
        ...
        <li class="card bg-base-100 shadow-sm">
          <div class="card-body p-4">
            ...
            <div class="flex flex-wrap gap-2 text-sm opacity-70">
              ...
              <span>By <a href={`https://news.ycombinator.com/user?id=${s.by}`} target="_blank" rel="noopener noreferrer" class="hover:underline">{s.by}</a></span>
              {#if s.time}
                <span>{formatDate(new Date(s.time * 1000).toISOString())}</span>
              {/if}
              {#if s.descendants !== undefined}
                <span><a href={`https://news.ycombinator.com/item?id=${s.id}`} target="_blank" rel="noopener noreferrer" class="hover:underline">{s.descendants} Comments</a></span>
              {/if}
            </div>
          </div>
        </li>
